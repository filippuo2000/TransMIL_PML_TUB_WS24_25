...
class SelfNystromAttention(nn.Module):
    ...
    def forward(self, x):
        x_norm = self.norm(x)
        # x = x + self.att(x_norm)
        out, att = self.att(x_norm, return_attn=True)
        x = x + out
        return x
...
class TransMIL(nn.Module):
    ...
    def forward(self, x):
        ...
        n = x.shape[1]
        N = np.ceil(np.sqrt(n))
        height_ppeg, width_ppeg = int(N), int(N)
        M = int(N**2 - n)
        ...
        if self.use_ppeg:
            # PPEG module
            x = self.ppeg(x, height_ppeg, width_ppeg)
