...
def plot_stats(epochs, y_vals, plot_name, plot_type = ["loss", "acc"], save=False, save_path=""):
    ...
    if save:
        save_path = Path(save_path, f"{plot_type}_{plot_name}_{epochs}_epochs.png")
        plt.savefig(save_path, bbox_inches="tight", dpi=300)  # High resolution and tight layout
        print(f"Plot saved to {save_path}")
    plt.show()
    plt.clf()

def visualize_slide_with_patches(slide_id: str = None, outfile: Path = None, base_dir="/mnt/"):
    # Prepare path names
    if slide_id:
        slide_id = slide_id
        # WSI filepath
        wsi_path = Path(base_dir, "slides", f"{slide_id}.tif")
        # WSI's patches datafile - containing information about their position in the WSI
        csv_path = Path(base_dir, "patches", "20x", slide_id, "metadata", "df.csv")
    ...
    wsi_label = df_labels['label'].loc[df_labels['case_id'] == slide_id].tolist()[0]
    ...
    wsi_thumbnail = slide.read_region(location=(0, 0), level=level, size=wsi_dimensions)
    ...
    for i, row in selected_patches.iterrows():
        ...
        position_abs = eval(row["position_abs"])  # Example: "(512, 10240)" -> (512, 10240)
        ...
        # Draw the rectangle
        rect = patches.Rectangle((x_scaled, y_scaled), patch_size_scaled, patch_size_scaled,
                                    linewidth=2, edgecolor='red', facecolor='none')
        plt.gca().add_patch(rect)
        plt.text(x_scaled, y_scaled - 5, f"{i+1}", color='black', fontsize=10)

    plt.title(f"Rotated WSI with Selected Patches Highlighted - slide {slide_id}, label: {label}")
    if outfile:
        plt.savefig(outfile, dpi=300, bbox_inches='tight')
    plt.show()

    # Show single patches randomly chosen earlier
    show_patches(patch_ids=selected_patches_ids, rows=1, cols=5, outfile=None)


def show_patches(patch_ids: List[str], rows: int, cols: int, outfile: Path = None):
    if len(patch_ids != rows*cols):
        raise ValueError(f"{len(patch_ids)} patches cannot be ploted in {rows} rows and {cols} columns grid")
    imgs = patch_ids
